...
/**
 * 공통으로 적용될 범위 설정
 * - org.koreait 패키지를 포함한 하위 패키지의 @Controller 애노테이션 적용된 모든 컨트롤러
 */
...
public class CommonControllerAdvice {
    ...
    public ModelAndView errorHandler(Exception e, Model model, HttpServletRequest request) {
        ...
        String tpl = "error/errorPage"; // 템플릿 경로 설정
        ...
        if (e instanceof CommonException commonException) { // 직접 정의한 예외인 경우 정의한 예외로 대체
            ...
            if (e instanceof AlertException) { // AlertException의 하위 클래스이면 아래 내용을 수행
            ...
            if (e instanceof AlertBackException alertBackException) {
                String target = alertBackException.getTarget(); // target - 이동할 창의 위치, 기본은 self
